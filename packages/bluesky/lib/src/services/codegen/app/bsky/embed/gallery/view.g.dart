// GENERATED CODE - DO NOT MODIFY BY HAND

// ignore_for_file: non_constant_identifier_names

part of 'view.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

_$EmbedGalleryViewImpl _$$EmbedGalleryViewImplFromJson(Map json) =>
    $checkedCreate(
      r'_$EmbedGalleryViewImpl',
      json,
      ($checkedConvert) {
        final val = _$EmbedGalleryViewImpl(
          $type: $checkedConvert(
              r'$type', (v) => v as String? ?? 'app.bsky.embed.images#view'),
          items: $checkedConvert(
              'items',
              (v) => (v as List<dynamic>)
                  .map((e) => const EmbedGalleryViewImageConverter()
                      .fromJson(e as Map<String, dynamic>))
                  .toList()),
          $unknown: $checkedConvert(
              r'$unknown',
              (v) => (v as Map?)?.map(
                    (k, e) => MapEntry(k as String, e),
                  )),
        );
        return val;
      },
    );

Map<String, dynamic> _$$EmbedGalleryViewImplToJson(
        _$EmbedGalleryViewImpl instance) =>
    <String, dynamic>{
      r'$type': instance.$type,
      'items': instance.items
          .map(const EmbedGalleryViewImageConverter().toJson)
          .toList(),
      if (instance.$unknown case final value?) r'$unknown': value,
    };
